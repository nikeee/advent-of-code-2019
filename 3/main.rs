...
fn main() -> Result<(), Box<dyn Error>> {
	...
	// Part 1
	let min_distance_to_origin = line_0_step_map.iter()
	...
	// Part 2
	let min_distance_in_steps = line_0_step_map.iter()
			.filter_map(|(point, line_0_steps)| line_1_step_map.get(point).map(|line_1_steps| (line_0_steps, line_1_steps)))
			.map(|(c0, c1)| c0 + c1)
			.min();

	match min_distance_in_steps {
		Some(distance) => println!("Part 2: The fewest combined steps the wires must take to reach an intersection: {}", distance),
...
fn get_visited_points(paths: Vec<Path>, origin: Point) -> HashMap<Point, u32> {
	use Direction::*;
